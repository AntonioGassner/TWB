<xsl:stylesheet
	...
	<xsl:variable name="one-hand-axe" select="concat(
                    'persian_axe_head,',
			        'persian_pick_head,',
                    'persian_handle_2,',
                    'persian_handle_1'
	...
	<xsl:variable name="one-handed-sword-pieces" select="concat(
                    'celticantennae1,',
                    'celticantennae2,',
                    'celticblade1,',
                    'celticblade2,',
	...
	<xsl:template match="//WeaponDescription[@id='TwoHandedMace']//AvailablePiece[not(following-sibling::AvailablePiece)]">
		...
		<xsl:template match="//WeaponDescription[@id='OneHandedSword']//AvailablePiece[not(following-sibling::AvailablePiece)]">
        <xsl:call-template name="generate-available-pieces">
            <xsl:with-param name="weapon-pieces">
                <xsl:value-of select="$one-handed-sword-pieces"/>
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
	...
	<xsl:template match="//WeaponDescription[@id='OneHandedAxe']//AvailablePiece[not(following-sibling::AvailablePiece)]">
		<xsl:call-template name="generate-available-pieces">
			<xsl:with-param name="weapon-pieces">
				<xsl:value-of select="$one-hand-axe"/>
			</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
